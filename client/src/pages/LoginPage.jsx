...
export default function LoginPage() {
  ...
  const handleGoogleCallback = async ({ credential }) => {
    ...
    catch (e) { setError(e.response?.data?.error || t.auth.googleSignIn + ' failed.') }
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <p style={styles.sub}>{t.auth.signInSubtitle}</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="btn btn-primary" style={{ width: '100%', marginTop: 4 }} disabled={busy}>
            {busy ? <><span className="spinner" style={{ width: 14, height: 14, borderWidth: 2 }} /> {t.auth.signingIn}</> : t.auth.signIn}
          </button>
        </form>

        <p style={styles.footer}>
          {t.auth.noAccount}{' '}
          <Link to="/register" style={{ color: 'var(--primary)', fontWeight: 600 }}>{t.auth.createOne}</Link>
        </p>
      </div>
    </div>
  )
